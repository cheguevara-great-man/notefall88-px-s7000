# 最终硬件与接线

## 只买最终会使用的东西

| 项目 | 数量 | 淘宝/电商搜索词 | 参考价格区间（人民币） |
|---|---:|---|---:|
| APA102C 或 SK9822 灯带 | 连续 2 m ×1 | `5V 144灯 APA102 SK9822 黑板 IP30 非防水` | 120–260 |
| ESP32-S3-DevKitC-1-N8R8 | 1 | 已有；乐鑫官方板 | 0（已有） |
| 74AHCT125 成品模块 | 1 | `74AHCT125 四路 电平转换 模块 3.3V 5V` | 8–25 |
| 5 V / 5 A 成品电源 | 1 | `5V5A 3C 电源适配器 DC5.5 2.1` | 35–80 |
| DC5.5×2.1 一进三出配电线 | 1 | 按 `docs/harness.csv` 定制，20 AWG | 15–35 |
| 3 A 带座保险丝 + 2 只备件 | 1 套 | `3A 低压 保险丝座 线束` | 7–20 |
| 1000 µF / 10 V 低 ESR 电容 | 2 | `1000UF 10V 低ESR 电解电容 引线` | 4–12 |
| Micro-USB 供电 OTG Y 线 | 1 | `Micro USB OTG 一分二 USB母 独立供电` | 已有或 5–20 |
| USB-A 公转 USB-B 公数据线 | 1 | `打印机数据线 USB A公 B公` | 10–25 |
| Micro-Fit 3.0 兼容 4P 锁扣线 | 1 对 | `Micro-Fit 3.0 4P 公母 预压线` | 15–35 |
| XT30 预焊 20 AWG 线 | 1 对 | `XT30 公母 带线 20AWG` | 8–15 |
| 20–22 AWG 红黑线、26–28 AWG 信号线 | 若干 | `硅胶线 20AWG 红黑`、`四芯 26AWG` | 15–35 |
| 黑色 PET/PVC 薄载体 | 1（可选） | `黑色 PET 硬片 0.8mm 裁条` | 10–30 |
| 拉伸移除式无痕固定条 | 3–5 片 | `无痕固定条 拉伸移除` | 10–30 |
| 控制盒 | 1（可选） | 成品通风塑料盒或约 60 g 黑色 PETG 打印件 | 15–40 |

完整逐项物料和代焊要求以 [BOM CSV](bom.csv) 与 [线束制造表](harness.csv) 为准。通用重新采购规格仍优先选择连续 2 m 灯带并只裁一次；本机已购实物已经是 `32 + 72 + 72` 三段焊接的 176 灯逻辑链，不再裁剪或重焊，生成映射已经纳入两个焊缝。琴键侧没有 3D 打印盒或扩散条。

## USB 三条连接

```text
普通打印机线 USB-B 端 ─────────────> PX-S7000 的 USB TO HOST
普通打印机线 USB-A 端 ─────────────> OTG Y 线的 USB-A 母口
OTG Y 线 Micro-USB 公头 ───────────> ESP32 上标 USB / OTG 的 Micro-USB 口
H5 电源线 Micro-USB 公头 ──────────> ESP32 上标 UART / USB-to-UART 的 Micro-USB 口
H7 电源线 Micro-USB 公头 ──────────> OTG Y 线第 3 个 Micro-USB 供电口
电脑数据线 ────────────────────────> 首次刷机时替换 H5 接 UART 口；刷完拔电脑线再插回 H5
```

不要把 OTG Y 线插到开发板的 `UART` 口，也不要把 H5 电源线插到 `USB/OTG` 口。PX-S7000 连接的是方形 USB-B `USB TO HOST` 口，而不是安装 WU-BT10 的 USB-A 口。

乐鑫官方 [ESP32-S3-DevKitC-1 用户指南](https://docs.espressif.com/projects/esp-dev-kits/en/latest/esp32s3/esp32-s3-devkitc-1/user_guide_v1.1.html) 明确说明 USB-to-UART 口可给开发板供电。官方 [V1.1 原理图](https://dl.espressif.com/dl/schematics/SCH_ESP32-S3-DevKitC-1_V1.1_20221130.pdf) 还显示两个 USB 口的 VBUS 分别经 D1、D7 肖特基二极管进入板上 5V 网，因此 H5 从 UART 口给板子上电时，原生 `USB/OTG` 口不会反向输出设备 VBUS。本设计让 H5 给开发板供电，同时用同一保险后 5V 的 H7 给 Y 线第 3 口供电，为钢琴 USB 设备侧提供 VBUS。商品所称“不支持对手机供电”意味着 H7 不能替代 H5，而不是第 3 口无用。该结论的完整理由、禁止接法和验收项固定在 [ADR-001](decisions/001-native-usb-host-and-vbus.md)。

这是一条双向 USB-MIDI 链路：钢琴把按键/踏板发送给 ESP32，ESP32 也可把“跟随我”模式的另一只手音符送回钢琴音源。Casio [PX-S7000 用户手册](https://www.casio.com/content/dam/casio/global/support/manuals/electronic-musical-instruments/pdf/008-en/p/PXS7000_usersguide_EN.pdf) 明确说明该接口可发送和接收演奏 MIDI；固件仍会在每次连接时独立探测 OUT 端点，没有 OUT 时不盲目发送。

## 灯带接线

```text
ESP32 GPIO11 ─> 74AHCT125 A1 ─> Y1 ─> 100 Ω ─> 灯带 DATA IN
ESP32 GPIO12 ─> 74AHCT125 A2 ─> Y2 ─> 100 Ω ─> 灯带 CLOCK IN
ESP32 GND ────────────────────────────────┐
74AHCT125 GND ────────────────────────────┼─> 灯带 GND
5 V / 5 A 电源负极 ──────────────────────┘
5 V / 5 A 电源正极 ─> 3 A 保险 ─┬─> 灯带左右两端 5V
                                  ├─> H5 → ESP32 USB-to-UART 供电口
                                  ├─> H7 → OTG Y 线第 3 供电口
                                  └─> 74AHCT125 VCC
5 V / 5 A 电源负极 ─────────────> 灯带左右两端 GND
```

74AHCT125 的 `/OE1`、`/OE2` 接 GND；未使用通道的输入固定接 GND、`/OE` 接 5 V。灯带首尾各并联一只 1000 µF 电容，注意负极条纹接 GND。整机只使用一个 5 V 电源，保险丝保护灯带、控制器和 USB Host 供电全部支路，避免两只 5 V 适配器互相回灌或依赖上电顺序。

![生产线束图](wiring-harness.svg)

灯带箭头必须从 `DATA IN/CLOCK IN` 指向输出端。首尾两端只并联供电，不把末端数据接回首端。

## 免焊接现实边界

开发板、OTG 和电源均可直接插接；唯一强烈建议焊接的是灯带裁切端、首尾注电和两根 100 Ω 信号线。可让淘宝灯带卖家或手机维修店按接线图预焊，要求套热缩管并做导通/短路检查。高电流供电不建议长期依赖无锁扣的杜邦线或廉价灯带夹。
