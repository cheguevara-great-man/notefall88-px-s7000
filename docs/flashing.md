...
首次成功打开网页后，进入“灯带校准 → 设备维护”，连接 `NoteFall-88` 热点并把默认热点密码改为至少 12 位的独有密码。密码保存在 ESP32 NVS，不会由诊断接口或网页回显；忘记时仍可用 UART 重新烧录/清除 NVS 恢复。
...
手机或平板连接 `NoteFall-88`，首次密码 `notefall88`，浏览器打开 `http://192.168.4.1`。导入标准 MIDI 文件即可。设置页可把 ESP32 加入家中 Wi-Fi；保存后会重启，热点仍保留作为故障恢复入口。
