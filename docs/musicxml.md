...
- 升降音、八度、chromatic + octave-change transpose、速度变化；`metronome` 的 beat-unit、附点和 per-minute 会换算为四分音符 BPM；
...
- 复杂 metric modulation（`metronome-note`/`metronome-relation`）、渐变速度曲线和力度发卡的连续插值；其离散起点仍可显示，但不合成连续曲线；
- 仅靠复杂文字/图形语义表达的演奏技巧；
...
数字测试使用 `web/test-fixtures/parser-etude.musicxml` 覆盖双谱表、和弦、backup、速度变化、升号和跨小节 tie；`meter-tempo-dynamics.musicxml` 覆盖加法/复合拍号、附点速度、力度和静默 cue。发布前还要用至少以下真实导出源各验证 3 首：MuseScore、Dorico/Finale 任一、以及常见网络 MXL。每首逐项比对：显示小节数、目标音高、首末时刻、速度变化、左右手和 tie，不只做“能打开”测试。
