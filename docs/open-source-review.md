...
关键区别是：**琴键到实体灯的实时路径走钢琴到 ESP32 的 USB 直连，不经过 Wi-Fi。** ESP 仍把带 USB 回调时间戳的按键事件广播给界面；网页用最低 RTT 时钟同步恢复硬件采集时刻，所以网络到达抖动不直接污染判分与录音。灯带采用带时钟线的 APA102/SK9822，而不是参考项目中常见的 WS2812；这让刷新时序更可控，也更适合 ESP32-S3 同时处理 USB 和网络任务。
...
| NoteFall 88 当前工程基线 | 本仓库 | MIT Core；Studio 组合发行物 GPL-3.0 | 超过 8,500 行非空原创代码与测试；345 项自动测试（71 Python/工程 + 274 Web，含固件共用纯 C++ MIDI/USB 描述符核心的主机编译执行），显式 Web 模块覆盖率门限进入 CI，另有 1016 文件 MusicXML/MXL 固定语料审计、Studio 本地 Worker 高级格式转换和 MIDI 五线谱伴侣、ESP/浏览器单调时钟同步、逐和弦局部拍速自适应判定与快速同音防串匹配、目标力度基准/轮廓、物理松键/CC64 发声时值、和弦展开/双手同步、MusicXML 踏板提前提示/时机/半踏深度评分与专项关卡、乐谱所选声部示范和录音立即回放、48 小节真浏览器自动翻谱、逐音/反复安全谱面光标、可点击的小节热力带、循证弱点巡回、跨曲目间隔复习队列、全曲指纹、跨双手和弦星座、目标力度与早/晚反馈 Canvas 像素门禁；控制面采用 SoftAP 自动授权、STA 会话再认证与默认密码验收门禁 |
...
| 循环、速度、左右手、移调 | ● | ● | ◐ | — | ● | ● |
| 录制与回放 | — | ● | — | 提供事件能力 | ● | ●，标准 MIDI 导出 + PX-S7000 直接回放 |
| 乐谱所选声部示范 | ● | ◐ | ◐ | 提供事件能力 | ● | ●，钢琴音源/静音预览双路径 |
| 逐音节奏/力度洞察与本地历史 | ◐ 得分/统计 | ◐ | — | — | ◐ | ●，有符号时序、P95、力度基准/轮廓/趋势、难点键、版本化导出 |
| 物理松键、踏板发声时值与释放训练 | — | ◐ 录制 Note Off/CC | — | — | — | ●，CC64 分离、断奏门控、复盘/趋势/教练/关卡 |
| 和弦展开与左右手同步训练 | — | — | — | ◐ 和弦聚合 | ◐ 视觉和弦 | ●，完整和弦评分、方向诊断、复盘/趋势/教练/关卡 |
| MusicXML 踏板时机与半踏深度训练 | — | — | — | — | — | ●，逐动作/逐遍评分、复盘/趋势/教练/关卡 |
| 可解释的自适应练习建议 | — | — | — | — | — | ●，SHA-256 同谱历史驱动片段/声部/模式/速度，一键应用且同名文件不串曲 |
| 多片段掌握路线与连续达标进阶 | — | — | — | — | — | ●，反复展开小节聚合、非重叠关卡、失败清零与本机恢复 |
