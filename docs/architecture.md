...
- 自适应教练用原始乐谱 SHA-256 而不是易冲突的曲名识别同一乐谱，最多读取最近 20 次记录；再用加权错漏密度选择 4–12 秒难点窗口，用左右手独立错误率选择声部，并按准确率/时序误差在 50/75/100/125/150% 档位间调整速度。建议始终显示证据量和置信度，用户一键应用后才改变练习设置。
