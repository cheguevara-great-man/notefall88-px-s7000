...
- 全谱示范和录音回放复用同一 MIDI OUT 所有权与急停边界，但各自使用有界的 1.5 秒前瞻时间窗。Note On、Note Off 和踏板按各自真实时刻分窗排程，因此长音不会被 60 秒单事件延迟上限截断。示范时网页仍推进谱面、瀑布流和目标灯；录音回放则保留原 MIDI 通道和 CC64/66/67，且两者均不进入练习评分或新录音。
- MIDI OUT 是受保护的写操作：设备存在 OUT 端点仍不等于当前页面可用。SoftAP 需自动授权，家庭 Wi-Fi 需先用热点密码换取当前标签页令牌；未授权页面会显示“需先解锁控制”，不会把端点存在误报成可播放。
...
88 键并非等距半音。映射生成器先按 52 个等宽白键建立白键中心，再把黑键中心放在相邻白键边界处，最后从实际物理坐标中选择最近 LED。当前 176 灯实物由 `32 + 72 + 72` 三段组成；段内名义节距 `6.944 mm`，31/32 与 103/104 缝按照片透视拟合分别额外加入 `4.22 / 3.98 mm`。重新生成后 88 键主像素仍全部唯一，最大理论中心误差为 `3.408 mm`。
...
- HTTP 更新写接口在家庭 LAN 与永久 SoftAP 上都要求不回显的管理密码请求头；固件身份检查、双槽 pending-verify、内部自检、真实客户端确认与 90 秒自动回滚共同保护更新。
- WebSocket 控制在 SoftAP 内自动授权；家庭 Wi-Fi/STA 首次在 `hello` 中验证管理密码。网页之后长期保存随机控制令牌，令牌也保存在 ESP NVS，因此浏览器关闭或 ESP 重启后无需重复输入；修改管理密码会换发令牌。重叠子网按 STA 失败关闭处理。
