...
describe("app preferences", () => {
  ...
  it("normalizes ranges and an invalid Follow Me hand", () => {
    ...
    expect(loadPreferences(source)).toEqual({
      ...
      navLoopMode: "loop", libraryFormatFilter: "all",
    });
  });

  it("round-trips supported values including notation type and nav loop mode", () => {
    const target = storage();
    savePreferences({
      version: 1,
      mode: "realtime",
      hand: "left",
      timingProfile: "strict",
      tempo: 0.75,
      leadMs: 1_200,
      previewSeconds: 2.8,
      autoFullscreen: true,
      metronome: true,
      countIn: true,
      sheetNotationType: "staff",
      navLoopMode: "measure", libraryFormatFilter: "all",
    }, target);
    expect(loadPreferences(target)).toMatchObject({
      mode: "realtime",
      hand: "left",
      timingProfile: "strict",
      tempo: 0.75,
  ...
  it("migrates existing v1 preferences to the standard visual horizon", () => {
    ...
    expect(loadPreferences(source)).toMatchObject({ timingProfile: "adaptive", previewSeconds: 4.2, autoFullscreen: false, sheetNotationType: "jianpu", navLoopMode: "loop", libraryFormatFilter: "all" });
  });

  it("persists five-percent tempo steps across the full practice range", () => {
    const target = storage();
    savePreferences({
      version: 1, mode: "wait", hand: "both", timingProfile: "adaptive", tempo: 0.35, leadMs: 900, previewSeconds: 4.2, autoFullscreen: false, metronome: false, countIn: true, sheetNotationType: "jianpu", navLoopMode: "loop", libraryFormatFilter: "all",
    }, target);
    expect(loadPreferences(target).tempo).toBeCloseTo(0.35);
    savePreferences({
      version: 1, mode: "wait", hand: "both", timingProfile: "adaptive", tempo: 2, leadMs: 900, previewSeconds: 4.2, autoFullscreen: false, metronome: false, countIn: true, sheetNotationType: "jianpu", navLoopMode: "loop", libraryFormatFilter: "all",
    }, target);
