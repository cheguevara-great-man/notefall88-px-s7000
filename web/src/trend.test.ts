...
function session(index: number, accuracy: number, timing?: number, dynamicsScore?: number, durationCoverageScore?: number): PracticeSession {
  return {
    ...
    summary: {
      ...
      dynamicsScore, durationCoverageScore, bestStreak: 10, problemNotes: [],
    },
...
describe("practice trend", () => {
  it("sorts attempts and compares stable early and late windows", () => {
    const trend = practiceTrend([
      session(4, 92, 55, 90, 95), session(1, 70, 120, 40, 55),
      session(3, 88, 70, 80, 90), session(2, 78, 95, 60, 65),
    ]);
