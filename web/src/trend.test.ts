...
function session(
  ...
  return {
    ...
    summary: {
      ...
      dynamicsScore, durationCoverageScore, coordinationScore, pedalScore, bestStreak: 10, problemNotes: [],
    },
...
describe("practice trend", () => {
  it("sorts attempts and compares stable early and late windows", () => {
    const trend = practiceTrend([
      session(4, 92, 55, 90, 95, 94, 96), session(1, 70, 120, 40, 55, 50, 42),
      session(3, 88, 70, 80, 90, 86, 88), session(2, 78, 95, 60, 65, 62, 60),
    ]);
    ...
    expect(trend.pedalDelta).toBe(41);
  ...
  it("does not infer progress from a single attempt", () => {
    ...
    expect(trend.pedalDelta).toBeUndefined();
  });
});
