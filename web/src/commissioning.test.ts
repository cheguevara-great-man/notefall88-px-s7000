...
describe("commissioning evidence", () => {
  ...
  it("records USB status and a real middle-C event, then completes", () => {
    ...
    state = observeDevice(state, {
      ...
      firmware: "0.6.0", protocol: 5, usbEndpoint: 0x81, usbOutEndpoint: 0x02,
      usbPackets: 42, usbErrors: 0, usbVid: 0x07cf, usbPid: 0x6803,
    });
    ...
    expect(commissioningReport(completed)).toMatchObject({ complete: true, missing: [], state: { completedFirmware: "0.6.0" } });
  });
