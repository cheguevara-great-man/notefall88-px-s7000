...
网页端可直接导入 MIDI、MusicXML、XML 和 MXL，按需显示瀑布流或五线谱，并提供实时、等我弹和跟随我三种练习模式、左右手筛选、25%–200% 五个百分点步进变速、±12 半音同步移调、A–B 循环和命中/错键/漏键统计。MusicXML 的小节轨既可两点设整小节 A–B，也可从已选小节开始一轮新的、不会把前段误记为漏音的练习。实时模式可启用按 MIDI/MusicXML 真实拍号、速度变化和反复顺序生成的 Web Audio 节拍器，以及一小节预备拍；常用练习设置刷新后保留。实时分析还会计算拍点早晚、平均/P95 时序误差、力度均值与波动、最长连续命中和逐键难点；瀑布流在对应琴键上即时给出命中、错键、漏键标记。每次练习连同原始乐谱 SHA-256、模式、声部、速度、移调与循环范围仅保存在本机，可导出 JSON；任意历史会话可重新打开为时间/键位复盘，且同曲目多次记录会以稳定前后窗口显示准确率与时序改善趋势。自适应教练按内容身份而非曲名汇总同谱近期历史，避免同名不同谱串曲，再找出错漏最集中的片段和较弱声部，给出有证据的模式、速度、A–B 建议并可一键应用；进一步的“弱点巡回”会按反复展开后的小节、近期权重、错漏、拍点和左右手差异生成最多三关不重叠的训练路线，自动配置整小节循环、模式、声部与速度，并要求每关连续达标而不是偶然弹对；进度只保存在本机，可中断恢复。跟随我模式只要求用户弹一只手，另一只手经带时间戳的 USB-MIDI OUT 队列由 PX-S7000 自身音源演奏；若实机没有 OUT 端点则明确降级为无伴奏跟随。IndexedDB 曲库支持文件夹、并发内容去重，以及带 SHA-256、规模上限和原子恢复的完整备份；演奏录制包含力度、MIDI 通道和延音踏板，可导出标准 MIDI。钢琴按键数据走 USB 直达 ESP32，因此按键反馈不依赖无线链路。ESP32 自动连接已保存的家庭 Wi-Fi，同时永久保留 `NoteFall-88` 救援热点；Android Studio App 在局域网内自动发现设备并在地址变化后重连，浏览器可使用当前家庭 IP 或路由器支持时的 `http://notefall.local`，永久救援入口始终是 `http://192.168.4.1/recovery`。

曲库中的“今日练习”把单曲教练提升到跨曲目：只接受覆盖至少 60% 目标音、且没有 A–B 循环的全曲记录作为掌握证据，薄弱曲优先，连续稳定后按 1/3/7/14/30/60 天逐级安排复习。短循环刷出的 100% 不会延长整曲复习间隔；全部计算和历史仍只留在本机。

“示范当前声部”可从当前位置按所选左手、右手或双手，将乐谱音符和 CC64 踏板用 1.5 秒有界前瞻队列送回 PX-S7000 自身音源；变速、A–B 循环、换踏和重复同音都使用同一时间线。没有 MIDI OUT 时仍能静音预览谱面、瀑布流和实体目标灯；页面进后台、断连、循环回跳和手动停止都会执行踏板释放与 16 通道急停。演奏录制完成后也可不经导出直接用钢琴回放：保留 MIDI 通道、力度和 CC64/66/67，不把音量、音色等无关控制回写到钢琴，长音的 Note Off 按后续时间窗精确排程而不被 60 秒协议上限提前截断。
...
实时判定与演奏录音使用 ESP32 在 USB 回调处采集的单调时间戳；浏览器通过低往返样本同步两端时钟，因此 Wi-Fi 排队抖动不会直接变成虚假的早/晚误差。旧固件或异常时间戳会明确降级为到达时间，不伪造同步精度。

实时模式不再用一套固定宽容度覆盖整首曲子：自适应、宽容、严格三档会按每个位置的真实拍速生成逐和弦窗口；快速重复同音还会受相邻同音间隔约束，避免一次按键误认成前后两个音。改变练习速度会保持相同的音乐拍值比例，练习面板同时显示当前现实毫秒范围。

录制导出会保留手指真实离键时刻、CC88 高分辨率力度前缀以及连续 CC64 半踏板、CC66/67 控制；实时练习分析在钢琴启用该选项时使用完整 14 位力度。若停止录制时延音踏板仍踩下，会自动补一个 CC64=0，避免文件回放时挂住踏板。

每个正确音还保留谱面目标力度。表现力分析会把整台琴/演奏者普遍偏轻或偏重的“力度基准”与相对强弱轮廓分开：前者单独显示，后者以 0–100 分进入历史趋势、复盘时间段和教练建议；谱面没有足够强弱变化时不会伪造轮廓分。

实时模式还会在 Note On 命中后继续跟踪物理 Note Off 与 CC64：分别记录手指松键和踏板抬起后的实际收音，计算谱面时值覆盖、无踏板释放精度、提前收音率与踏板延长量。MusicXML 的 staccato、staccatissimo、detached-legato 和 tenuto 会修正目标门控；没有可靠踏板记号时只报告踏板延长，不把过踏板武断判成错误。

和弦同步分析只合并乐谱中起点完全相同且完整命中的音，不把琶音、邻近快速音或漏音后的残缺和弦误判为“不齐”。它分别给出和弦展开宽度、P95、整齐度分，以及右手相对左手的有符号提前/滞后；这些证据进入时间复盘、历史趋势、教练调速和带连续达标门槛的双手专项巡回。

MusicXML 谱面中的延音踏板开始、停止、换踏和 0–100% 半踏深度会展开到真实反复播放时间线，先以 `PED ↓ / ↑ / ↻ / 百分比` 在 Web Canvas 与 Android 原生瀑布流中提前提示，再与 PX-S7000 的连续 CC64 输入逐动作匹配。判定按当前速度换算实际毫秒偏差，A–B 重复逐遍隔离，区分漏踏、多余动作、时机和深度；结果进入实时洞察、分段复盘、历史趋势、自适应调速与实时踏板专项关卡。没有踏板记号的谱面不会伪造提示或踏板分数。
...
首次仍通过 UART 线刷；此后可从家庭 LAN 或永久救援热点用管理密码更新双槽固件或 LittleFS 网页。上传中断不会切换启动槽；新固件必须通过内部自检并被真实 HTTP/WebSocket 客户端访问，未确认或启动异常会自动回滚。固件内置不依赖 LittleFS 的 `/recovery`。家庭 Wi-Fi 上的控制首次用管理密码解锁，随后浏览器持久保存随机令牌且 ESP 重启后仍有效；修改管理密码会撤销旧令牌。详见 [安全更新说明](docs/update.md)、[网络与救援](docs/deploy/network-and-recovery.md) 与 [WebSocket 协议](docs/protocol.md)。
...
- 5 V、144 像素/米、IP30 的 APA102C 或 SK9822；当前实物为 `32 + 72 + 72` 三段焊成的一条逻辑链
- 共 176 像素；计入两个照片拟合焊缝后物理长度约 `1230.42 mm`，覆盖 1222 mm 的 88 键键床
- 74AHCT125 成品电平转换模块
